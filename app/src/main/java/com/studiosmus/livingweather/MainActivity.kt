...
import com.studiosmus.livingweather.databinding.ActivityMainBinding
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Restore saved background if any
        getSharedPreferences(LivingWeatherWallpaperService.PREFS, MODE_PRIVATE)
            .getString(LivingWeatherWallpaperService.KEY_BG, null)
    ...
    private fun handleImageUri(uri: Uri) {
        ...
        lifecycleScope.launch {
            ...
            if (path != null) {
                savedImagePath = path
                getSharedPreferences(LivingWeatherWallpaperService.PREFS, MODE_PRIVATE)
                    .edit().putString(LivingWeatherWallpaperService.KEY_BG, path).apply()
                updatePreview(path)
    ...
    private fun setLiveWallpaper() {
        val intent = Intent(WallpaperManager.ACTION_CHANGE_LIVE_WALLPAPER).apply {
            putExtra(
                ...
                ComponentName(packageName, LivingWeatherWallpaperService::class.java.name)
            )
        }
